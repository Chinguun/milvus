...
# Use ccache as compiler launcher to prevent unnecessary rebuilds
export CMAKE_C_COMPILER_LAUNCHER=ccache
export CMAKE_CXX_COMPILER_LAUNCHER=ccache

arch=$(uname -m)
if [ "${GPU_VERSION}" = "ON" ]; then
    CUDA_COMPILER_FLAG="-DCMAKE_CUDA_COMPILER=${CUDA_COMPILER}"
else
    CUDA_COMPILER_FLAG="-DCMAKE_CUDA_COMPILER="
fi
